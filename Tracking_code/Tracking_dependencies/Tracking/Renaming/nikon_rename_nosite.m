function nikon_rename_nosite(ORIGINAL_DIR, DESTINATION_DIR, varargin)
...
for d = 1:length(ORIGINAL_DIR)
    ...
    if ~isempty(files)
        ...
        currWell = getTokens(files,'Well([A-H]\d{2})_'); %Changed from 'Point' to 'Well'
%         currSite = cellfun(@str2num,getTokens(files,'Point[A-H]\d{2}_(\d+)'));
        ...
        for f = 1:length(files)
            ...
            new_dir = fullfile(DESTINATION_DIR,[wellNameToRowColumn(currWell{f}) '_1']);
